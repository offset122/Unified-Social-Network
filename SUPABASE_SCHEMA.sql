-- ============================================================
-- SOCIALAPP — COMPLETE SUPABASE SCHEMA
-- Run this in Supabase SQL Editor (Database → SQL Editor → Run)
...
create index if not exists posts_is_reel_idx    on public.posts(is_reel);
...
create table if not exists public.notifications (
  ...
  type       text not null check (type in (
    'follow','like','comment','reply','message','mention',
    'live','save','report','new_post','audio_call','video_call'
  )),
...
create index if not exists live_active_idx on public.live_sessions(is_active) where is_active = true;
create index if not exists live_host_idx on public.live_sessions(host_id);
...
create index if not exists live_msgs_session_idx on public.live_messages(session_id, created_at asc);
...
create index if not exists live_viewers_session_idx on public.live_viewers(session_id);
create index if not exists live_viewers_user_idx on public.live_viewers(user_id);
...
create index if not exists live_join_req_session_idx on public.live_join_requests(session_id, status);
create index if not exists live_join_req_requester_idx on public.live_join_requests(requester_id);
...
returns void language sql security definer as $$
  update public.live_sessions set viewers_count = viewers_count + 1 where id = session_id;
$$;

create or replace function public.decrement_live_viewers(session_id uuid)
returns void language sql security definer as $$
  update public.live_sessions set viewers_count = greatest(0, viewers_count - 1) where id = session_id;
$$;

create or replace function public.increment_live_likes(session_id uuid)
returns void language sql security definer as $$
  update public.live_sessions set likes_count = likes_count + 1 where id = session_id and is_active = true;
$$;

create or replace function public.decrement_live_likes(session_id uuid)
returns void language sql security definer as $$
  update public.live_sessions set likes_count = greatest(0, likes_count - 1) where id = session_id and is_active = true;
$$;

create or replace function public.upsert_live_viewer(p_session_id uuid, p_user_id uuid)
returns void language plpgsql security definer as $$
begin
  insert into public.live_viewers (session_id, user_id) values (p_session_id, p_user_id)
...
returns void language plpgsql security definer as $$
begin
  delete from public.live_viewers where session_id = p_session_id and user_id = p_user_id;
end;$$;

create or replace function public.accept_join_request(p_request_id uuid, p_session_id uuid)
returns void language plpgsql security definer as $$
begin
  update public.live_join_requests set status = 'accepted' where id = p_request_id and session_id = p_session_id;
end;$$;

create or replace function public.reject_join_request(p_request_id uuid, p_session_id uuid)
returns void language plpgsql security definer as $$
begin
  update public.live_join_requests set status = 'rejected' where id = p_request_id and session_id = p_session_id;
end;$$;

create or replace function public.increment_posts_count(user_id uuid)
returns void language sql security definer as $$
  update public.profiles set posts_count = posts_count + 1 where id = user_id;
$$;
...
create policy "convos_update" on public.conversations for update using (
  exists (select 1 from public.conversation_members where conversation_id = id and user_id = auth.uid() and is_admin = true)
);
...
create policy "storage_select" on storage.objects for select using (bucket_id in ('media','chat-media','avatars'));
create policy "storage_insert" on storage.objects for insert with check (auth.role() = 'authenticated' and bucket_id in ('media','chat-media','avatars'));
create policy "storage_update" on storage.objects for update using (auth.uid() = owner);
create policy "storage_delete" on storage.objects for delete using (auth.uid() = owner);
