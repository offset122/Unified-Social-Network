...
export default function CreateGroupScreen() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const { data: convo } = await supabase.from("conversations").insert({ type: "group", name: name.trim(), created_by: user.id }).select().single();
      if (!convo) throw new Error("Failed to create group");
      await supabase.from("conversation_members").insert({ conversation_id: convo.id, user_id: user.id, is_admin: true });
      router.replace({ pathname: `/chat/${convo.id}`, params: { peerName: name.trim() } } as any);
    } catch (e: any) {
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8, borderBottomColor: colors.border }]}>
        ...
        <Pressable onPress={handleCreate} disabled={!name.trim() || creating}
          style={[styles.createBtn, { backgroundColor: name.trim() ? colors.primary : colors.muted }]}>
      ...
      <View style={{ padding: 20 }}>
        ...
        <TextInput style={[styles.input, { color: colors.foreground, backgroundColor: colors.secondary, borderColor: colors.border }]}
          placeholder="Enter group name..." placeholderTextColor={colors.mutedForeground}
...
const styles = StyleSheet.create({
  ...
  createBtn: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 18 },
